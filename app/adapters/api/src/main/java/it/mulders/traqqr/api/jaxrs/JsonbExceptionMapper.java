...
public class JsonbExceptionMapper implements ExceptionMapper<JsonbException> {
    ...
    public Response toResponse(final JsonbException exception) {
        ...
        var message =
                switch (cause.getClass().getSimpleName()) {
                    case "DateTimeParseException":
                        ...
                        yield "Invalid date/time format '%s' at position %d"
                                .formatted(dtpe.getParsedString(), dtpe.getErrorIndex());
                    default:
        ...
        return Response.status(400).entity(result).type("application/json").build();
    }
