...
class LocationLookupJobStarterTest implements WithAssertions {
    ...
    private final LocationLookupJobStarter job = new LocationLookupJobStarter(jobOperator, wrapper);

    @Test
    void should_start_job_manually() {
        // Arrange
        var event = new JobStartRequestedEvent(BatchJobType.LOCATION_LOOKUP);

        // Act
        job.startManual(event);

        // Assert
        assertThat(this.jobOperator.getRequestedJobStarts())
                .anyMatch(rjs -> "location-lookup".equals(rjs.jobXMLName()));
    }
